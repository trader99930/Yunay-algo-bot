...
# ✅ STREAMLIT CONFIGURATION (MANDATORY: Must be the first Streamlit command)
st.set_page_config(page_title="Quantum Multi-User Terminal", layout="wide", initial_sidebar_state="collapsed")

# =====================================================
# 🔐 FYERS AUTO-LOGIN & TOKEN GENERATION (MERGED FROM fyers_login.py)
# =====================================================
FY_ID = "FS3309"
APP_ID_TYPE = "2"
...
def fyers_auto_generate_token():
    ...
    st.write("🔗 Injecting Token into bot's Database (users_database.json)...")
    try:
        users_db = load_users()
        TARGET_USER = "Yunay" 
        
        if TARGET_USER not in users_db:
            st.warning(f"⚠️ User '{TARGET_USER}' not found. Initializing profile...")
        ...
        users_db[TARGET_USER]["active"] = True  

        save_users(users_db)
        st.success(f"🎉 SUCCESS! Token safely injected for user: {TARGET_USER}")
        return True
    except Exception as e:
        st.error(f"❌ Database Injection Failed: {e}")
        return False

def show_login_page():
    st.title("🔐 Fyers Multi-User Login")
    
    # Naya advance button jo token banayega aur login karega
    if st.button("🚀 Generate Fyers Token & Login (Recommended)", type="primary"):
        with st.spinner("Connecting to Fyers API & Logging in..."):
            is_success = fyers_auto_generate_token()
            if is_success:
                time.sleep(1)
    ...
    # Aapka purana simple bypass button
    if st.button("Direct Login (Bypass Token Gen)"):
        st.session_state["logged_in"] = True
        st.rerun()
